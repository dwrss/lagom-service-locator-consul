  ...
  "A Consul-based service locator" should {
    ...
    "allow lookup of one service instance out of many" in withServiceDiscovery { locator =>
      client =>
        ...
        val registeredUrl = locator.locate("service-3").toCompletableFuture.get(
          testTimeoutInSeconds, TimeUnit.SECONDS
        )
        val expectedUrl = Optional.of(new URI(s"http://$localAddress:9003"))
        expectedUrl shouldBe registeredUrl
    ...
    "return CompletableFuture[Empty] for lookup of services that aren't registered" in withServiceDiscovery { locator =>
      client =>
